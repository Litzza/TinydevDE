...
// player0.position.needStamina = true; // default for Entities
...
player0.addMousePointerEvent((event, targetObject) => {
    ...
    // do not attack self
    if(targetObject.id == player0.id){
        return false; // deny: attack self (we should allow it only for debugging, since there are some bugs with it... this is important for multiple enemies or even multiplayer...)
    }
    
    // can be attacked: boolean
    if(!targetObject["isEntity"]) {
        return false;
    }

    // attack on cooldown (1)
    if(targetObject.lastFight > Date.now() - 1 * 1000) {
        console.warn("Fight is on Cooldown!");
        return;
    };

    // attack on cooldown (2)
    if(player0.lastFight > Date.now() - 1 * 1000) {
        console.warn("Fight is on Cooldown!");
        return;
    };

    // START FIGHT:
    if(!targetObject.isFighting && !player0.isFighting) {
        player0.removeKeyMovementController();
        player0.actionPressed = {...player0.keyConfig};
    
        player0.onFightStart(targetObject);
        player0.position = new Position(-100, 0);
        player0.battleClickCounter = 0;
    
        targetObject.onFightStart(player0);
        targetObject.position = new Position(100, -50);
        
    
        // switch scene on fight start:
        const fight_scene = new Scene();
        fight_scene.setBackgroundImage("./sources/Scenes/battle.png");
        fight_scene.appendChild(player0);
        fight_scene.appendChild(targetObject);
    
        GameEngine.scene = fight_scene;
        return;
    }


    // ATTACK:
    if(player0.battleTarget != targetObject){
        console.warn("wrong enemy", player0);
        console.warn("wrong enemy", player0.battleTarget);
        return;
    }
    const killed = player0.attack(targetObject);
    targetObject.position.set(Math.random() * innerWidth - innerWidth/2, Math.random() * innerHeight - innerHeight/2);
    player0.battleClickCounter++;
    

    // END FIGHT:
    if(killed) {
            console.error("YOU KILLED HIM - YOU MONSTER");

            // fight end: reset scene
            GameEngine.scene = scene;
            targetObject.onFightEnd();

            player0.onFightEnd();
            player0.addKeyMovementController();
...
const loader = new ImageLoader(() => {
    const player1 = spawnSlime(0, 0, 0.25);
    const player2 = spawnSlime(100, 0, 0.5);
    const player3 = spawnSlime(100, 100, 1);
    addGuiElements();
});

const img = loader.load("./sources/Player/player.png");
const img2 = loader.load("./sources/Mobs/slime.png");


window.addEventListener("load", () => {

    GameEngine.resume(); // default is pause, so the game starts whenever I want it.

});


function spawnSlime(x, z, scale) {
    const player1 = new EntityObject();
    player1.setImage("./sources/Mobs/slime.png");
    player1.scale = scale || Math.random() + 0.25;
    player1.position.set(x, z);
    player1.setName("Slime");
    player1.showHitbox = true;
    scene.appendChild(player1);

    const mobHpDynamic = new Bar(player1, "hp", "maxHp");
    mobHpDynamic.staticPosition = false;
    mobHpDynamic.position.set(0, -player1.height/2 - dynamicStatsOffset - 30);
    mobHpDynamic.valueColor = "red";
    player1.appendChild(mobHpDynamic);
    return player1;
}
...
function addGuiElements() {
    ...
    const battleClickCounterStatic = new Menu(player0, "battleClickCounter", "battleClickCounter");
    battleClickCounterStatic.staticPosition = false;
    battleClickCounterStatic.position.set(0, -player0.height/2 - dynamicStatsOffset - 55);
    battleClickCounterStatic.setSize(150, 10);
    battleClickCounterStatic.showValue = true;
    battleClickCounterStatic.showName = true;
    battleClickCounterStatic.valueColor = "green";
    player0.appendChild(battleClickCounterStatic);
    ...
    btn0.onclick = () => {scene.gravity = !scene.gravity; console.log(scene.gravity)};
